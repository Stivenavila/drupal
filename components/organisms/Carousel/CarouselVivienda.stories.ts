import type { Meta, StoryObj } from '@storybook/react'
import { CarouselVivienda as CarouselViviendaJ } from './CarouselVivienda'
import { dataTabsInfo } from '../../../common/seed/components'

const meta: Meta<typeof CarouselViviendaJ> = {
  title: 'Organisms/Carousel/CarouselVivienda',
  component: CarouselViviendaJ,
}

export default meta

type Story = StoryObj<typeof CarouselViviendaJ>

export const CarouselVivienda: Story = {
  args: {
    componenteTitle: {
      pSubtitle: 'Lorem',
      title: 'Proyectos disponibles',
    },
    carruselVivienda: {
      results: [
        {
          title: 'Colegio test Miraflores',
          livingPlaceLogo: {
            mediaImage: {
              url: 'https://picsum.photos/277/281',
              alt: 'images',
            },
          },
          livingPlaceSubtitle: {
            processed: 'Proyecto de <br> apartamentos desde: <strong>$189.750.000</strong>',
          },
          livingPlacePrice: {
            price: 187000000,
            subsidyApplies: true,
          },
          button: {
            url: {
              title: 'Más información',
              url: 'https://www.youtube.com/watch?v=g0rRsE3QfAE',
            },
          },
          livingPlaceTabs: { tabsInfo: dataTabsInfo },
        },
        {
          title: 'Colegio test Miraflores',
          livingPlaceLogo: {
            mediaImage: {
              url: 'https://picsum.photos/277/281',
              alt: 'images',
            },
          },
          livingPlaceSubtitle: {
            processed: 'Proyecto de <br> apartamentos desde: <strong>$189.750.000</strong>',
          },
          livingPlacePrice: {
            price: 187000000,
            subsidyApplies: true,
          },
          button: {
            url: {
              title: 'Más información',
              url: 'https://www.youtube.com/watch?v=g0rRsE3QfAE',
            },
          },
          livingPlaceTabs: { tabsInfo: dataTabsInfo },
        },
      ],
    },
  },
}
